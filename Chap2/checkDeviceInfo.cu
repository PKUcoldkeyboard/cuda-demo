...
int main(int argc, char **argv)
{
    ...
    log_info("<<< Total amount of global memory: %.2f Mbytes",
    ...
    log_info("<<< Total amount of constant memory: %.2f Mbytes",
             deviceProp.totalConstMem / pow(1024.0, 3));
    log_info("<<< Total amount of shared memory per block: %.2f Mbytes",
             deviceProp.sharedMemPerBlock / pow(1024.0, 3));
    log_info("<<< Total amount of registers avalible per block: %d", deviceProp.regsPerBlock);
    log_info("<<< Warp size: %d", deviceProp.warpSize);
    log_info("<<< Max number of threads per multiprocessor: %d",
